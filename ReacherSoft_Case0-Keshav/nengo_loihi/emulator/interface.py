...
class ProbeState:
    ...
    def send(self, probe, already_sent, receiver):
        ...
        if n_timesteps > 0:
            x = probe.weight_outputs(outputs)[-1]
            # adjust the time to fit with the output node only being called every n_timesteps
            time = self.dt * (already_sent + n_timesteps) / n_timesteps
            receiver.receive(time, x)
