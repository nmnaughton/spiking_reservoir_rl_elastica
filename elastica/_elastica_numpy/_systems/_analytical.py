__doc__ = "Analytically integrable systems, used primarily for testing time-steppers of Elastica Numpy implementation"

import numpy as np
from elastica._elastica_numpy._rotations import _get_rotation_matrix, _rotate
...
class BaseSymplecticSystem:
    ...
    def kinematic_rates(self, time, prefac, *args):
        return self._dyn_state * prefac
...
class SymplecticUndampedSimpleHarmonicOscillatorSystem(
    ...
    def dynamic_rates(self, time, prefac, *args, **kwargs):
        return (
            super(SymplecticUndampedSimpleHarmonicOscillatorSystem, self).__call__(
                time, *args, **kwargs
            )[-1]
            * prefac
        )
...
class SimpleSystemWithPositionsDirectors(_RodSymplecticStepperMixin):
    def __init__(self, start_position, end_position, start_director):
        ...
        self.omega_collection = np.lib.stride_tricks.as_strided(
            self.v_w_collection[1], (3, self.n_elems),
        )
        ...
        self.alpha_collection = np.lib.stride_tricks.as_strided(
            self.dvdt_dwdt_collection[1], (3, self.n_elems),
        )
